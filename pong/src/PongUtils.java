...
// variables gestion collisions

    private static final double UP_LIMIT = 15; // limite haut
    private static final double BOTTOM_LIMIT = 735; //limite bas 
    private static final double INITIAL_VX = 5.0; // vitesse initiale en X
    private static final double INITIAL_VY = 1.0; // vitesse initiale en Y
    private static final double SPEED_LIMIT = 30;
    private static final double ACCELERATION = 1.05;

    private static double angle = atan2(INITIAL_VY, INITIAL_VX); // angle de depart par rapport a X et Y initial
    private static double magnitude = sqrt(26); // racine carrée de (INI_VX au carré + INI_VY)

    // distance ajoutée à chaque tour de boucle Animation par rapport à al distance precedente
 ...
 // Permet de mettre à jour la position du joueur ET de ne pas sortir du board
    ...
    public static void updateGame(Rectangle player, Rectangle computer, Circle ball) {
         //ball.setCenterX(ball.getCenterX() + 4); ----> TEST 
    ...
    // On génère un getter (qu'on récupère dans le controller) pour le score et l'affichage pour le Binding 
    public static SimpleIntegerProperty GetscoreProperty() {
        return score;
    }
